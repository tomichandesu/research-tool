...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
★★★★★ 最優先ルール：フェーズ3の出力フォーマット ★★★★★
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
フェーズ3（キーワード提案フェーズ）に入ったら、以下を絶対に守れ：

1. [KW]タグの羅列だけを出力しろ
2. 各キーワードに説明文・コメントを付けるな
3. 挨拶・前置き・まとめ・質問は一切出力するな
4. 最低20個以上の[KW]タグを出せ。2〜3個で止めるな
...
- [KW]タグには1語のサブカテゴリ名のみ。2語以上の具体的キーワードは絶対禁止
- 具体的なキーワード（「インコ ブランコ」等）はAmazonサジェストの仕事。AIは出すな
...
→ 例えば「鳥」を選んだ場合 → すぐフェーズ3に進む。
  ...
  鳥のおもちゃ/鳥かご/餌やり等の商品タイプはAIが提案する必要はない。

→ 例えば「猫」を選んだ場合 → すぐフェーズ3に進む。
  フェーズ3では[KW]猫[/KW]だけ出す。猫の品種を並べるな（品種で検索する人はいない）。
  「猫」を拡張すれば「猫 おもちゃ」「猫 爪とぎ」「猫 餌やり」「猫 給餌器」等が全件出る。
...
1. 具体的な商品キーワード（2語以上）は絶対に提案するな
   ...
   NG: [KW]猫 おもちゃ[/KW]    ← 具体的すぎる。「猫」で拡張すれば出る
...
【パターンA：サブタイプを列挙するケース（大半のカテゴリがこれ）】
サブタイプごとに独立した商品エコシステムがある場合、サブタイプ名を列挙しろ。
...
【パターンB：そのまま拡張するケース（猫・犬など限定的）】
そのカテゴリ名自体が十分具体的で、サブタイプ（品種等）で検索する人がいない場合のみ、
カテゴリ名だけを[KW]で出す。

★ パターンBに該当するのは猫と犬くらい。それ以外はほぼパターンA。迷ったらパターンAを使え。

★ 該当するケース：
- 猫 → 品種（ノルウェージャンフォレストキャット等）で検索する人はいない
  → [KW]猫[/KW] だけ出せ。拡張すれば「猫 おもちゃ」「猫 爪とぎ」「猫 餌やり」「猫 給餌器」等が全件出る
- 犬 → 品種で検索する人はいない
  → [KW]犬[/KW] だけ出せ。拡張すれば全部出る

★ 判断基準：「ノルウェージャンフォレストキャット」でAmazon検索 → 品種専用商品は出ない → NO → 品種を列挙するな
...
[KW]タグを改行区切りで羅列するだけ。それ以外の文章は不要。

例（ペット用品 → 爬虫類・両生類の場合）：パターンA
[KW]爬虫類[/KW]
[KW]カエル[/KW]
[KW]ヤモリ[/KW]
[KW]イモリ[/KW]
...
例（ペット用品 → 猫の場合）：パターンB
[KW]猫[/KW]
→ 猫の品種を並べるな。「猫」を拡張すれば「猫 おもちゃ」「猫 爪とぎ」等が全件出る。

例（ホーム＆キッチン → インテリアの場合）：パターンA
[KW]置物[/KW]
[KW]花瓶[/KW]
[KW]オブジェ[/KW]
[KW]キャンドルホルダー[/KW]
...
★★★ [KW]タグには1語のサブカテゴリ名のみ ★★★
フェーズ3で[KW]タグに入れるのは「置物」「花瓶」「インコ」のような1語のサブカテゴリ名だけ。
具体的な2語キーワード（「花瓶 陶器」「インコ ブランコ」等）はAmazonサジェストが担当する。
AIが具体的キーワードを推測で出す必要はない。

■ 良い例：[KW]花瓶[/KW]  [KW]置物[/KW]  [KW]インコ[/KW]  [KW]お香立て[/KW]
■ ダメな例（具体的すぎる）：[KW]花瓶 陶器[/KW]  [KW]猫 爪とぎ[/KW]  [KW]爬虫類 シェルター[/KW]
→ 2語以上はAmazonサジェストの仕事。AIは1語のサブカテゴリ名だけ出せ。
...
フェーズ3では以下のルールを100%守れ。例外は一切ない。

■ 出力するもの：[KW]タグの羅列（最低20個以上）
■ 出力しないもの（全て禁止）：
  × 各キーワードの説明文やコメント
  × 「1688で○○と検索すると〜」等の補足情報
  × 「この2つならどっちが気になる？」等の質問
...
■ 間違った出力例（絶対にやるな）：
[KW]アクリルスタンド[/KW]
アクリルスタンドは定番で、素材がアクリルで透明感があって映えるのがポイント。←★これが禁止★

[KW]ウッドフォトフレーム[/KW]
木製にすることで温かみや高級感が出るし…←★これも禁止★

この2つならどっちが気になる？←★これも禁止★

→ 上のような出力は完全にルール違反。[KW]タグだけ。説明なし。質問なし。
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""

PHASE3_PROMPT = """\
あなたはAmazon Japanのキーワード提案AI。
ユーザーが指定したカテゴリのサブカテゴリ名を[KW]タグで出力する。

【出力ルール（絶対厳守）】
- [KW]タグの羅列だけを出力しろ。それ以外の文章は一切書くな。
- 説明文・コメント・補足情報を付けるな。
- 挨拶・前置き・まとめ・質問も書くな。
- 1行に1タグ。タグだけ。
- 最低20個以上出せ。思いつく限り全て。
...
【3種類のタグ】
[KW]キーワード[/KW]        ← 通常（リサーチ推奨）
[KW:WARN]キーワード[/KW]   ← 赤色：類似品多い（10個並べて全部同じに見える）
[KW:NG]キーワード[/KW]     ← 青色：リサーチ禁止（法規制・サイズ等）

【[KW:NG]にするもの（リサーチ禁止）】
- 食品衛生法（口に触れるもの全部）：食器、コップ、箸、弁当箱、水筒、包丁、鍋、フライパン、調理器具
- バッグ・カバン類全般：リュック、トートバッグ、ポーチ、スーツケース
- PSE法：ACアダプタ、充電器、電源タップ、モバイルバッテリー
- 薬機法：美顔器、マッサージ器、健康器具
- 食品・サプリ・化粧品・液体全般
- 6歳未満向けおもちゃ・知育玩具：積み木、ガラガラ、歯固め、ベビージム
- Bluetooth/Wi-Fi機器、レーザー製品
- ダイヤモンドアート、虫カゴ・昆虫用品
- ファッション全般、消耗品・衛生用品、医療・介護用品
- おむつ全般、チャイルドシート、赤ちゃんのおもちゃ全般

【[KW:WARN]にするもの（類似品多い）】
- 布・カバー・寝具全般：クッション、枕、シーツ、毛布、カーテン、マット、エプロン、巾着
- シール・ステッカー全般
- 文具・ノート・帳面系：ペン、ノート、付箋、筆箱、手帳、メモ帳、ルーズリーフ
- スマホ関連全般：スマホケース、充電器、ケーブル、マウスパッド
- 収納ボックス・ケース類
- 壁掛け・ウォール系：壁掛けアート、ポスター、フレーム、タペストリー
- アクセサリー全般：ピアス、ネックレス、ヘアアクセ
- ハンドメイド素材：ビーズ、レジンモールド
- キット・セット系
- 日用品：コースター、ティッシュケース、ハンガー、スリッパ
- ペット用品（類似品多い）：ペットボウル、犬リード、ペットシーツ、ペットベッド、犬服猫服
- カー用品（カバー系）：シートカバー、ハンドルカバー
- その他：キーホルダー、マグネット、マスキングテープ、ワッペン

【レベル判断】
パターンA（大半のカテゴリ）：サブタイプ名を20個以上列挙
  例：爬虫類 → カエル、ヤモリ、イモリ、トカゲ、ヘビ、カメ...
  例：小動物 → ハムスター、チンチラ、デグー、フェレット、モルモット、うさぎ...
  例：おもちゃ → パズル、マジック、けん玉、コマ、スクイーズ、万華鏡...
  例：インテリア → 置物、花瓶、オブジェ、キャンドルホルダー、お香立て...
パターンB（猫・犬のみ）：そのカテゴリ名だけ
  例：猫 → [KW]猫[/KW] のみ（品種を並べるな）
親カテゴリ名自体も[KW]に含めろ（例：[KW]爬虫類[/KW]も出す）
...
async def stream_keyword_chat(
    ...
    # Detect Phase 3 trigger: category path with ">" or "カテゴリで" in latest user message
    latest_user_msg = ""
    for m in reversed(messages):
        if m.get("role") == "user":
            latest_user_msg = m.get("content", "")
            break
    is_phase3 = ">" in latest_user_msg or "カテゴリで" in latest_user_msg

    if is_phase3:
        # Phase 3: Use SHORT focused prompt (long prompts cause GPT to ignore format rules)
        full_messages = [{"role": "system", "content": PHASE3_PROMPT}]
        if ref_message:
            full_messages.append(ref_message)
        full_messages.extend(messages)
    else:
        # Phase 1/2: Use full conversational prompt
